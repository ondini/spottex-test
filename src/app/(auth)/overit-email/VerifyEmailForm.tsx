...
export default function VerifyEmailForm({ token }: { token: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError("Ověření se nepodařilo. Otevřete prosím odkaz z e-mailu znovu.");
        return;
      }
      window.location.assign(response.url);
    } catch {
      setError("Ověření se nepodařilo. Zkuste to prosím znovu.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <form action="/api/auth/verify-email" method="post" onSubmit={submit} className="mt-8">
      ...
      <button type="submit" disabled={pending} className="app-button w-full disabled:opacity-60">
        ...
        {pending ? "Ověřuji…" : "Ověřit e-mail a aktivovat účet"}
      </button>
    </form>
  );
}
