...
describe("analysis worker recovery", () => {
  ...
  it("hands a freshly locked job back only when the worker forces it", async () => {
    // A worker shutting down releases its own job immediately; the periodic
    // stale-lock sweep must keep ignoring locks younger than the lock window.
    const suffix = randomUUID();
    const now = new Date();
    const user = await prisma.user.create({ data: { email: `analysis-release-${suffix}@example.test`, passwordHash: "not-used-by-test", status: "ACTIVE" } });
    const site = await prisma.energySite.create({ data: { userId: user.id, provider: "DEMO", externalSiteId: `analysis-release-${suffix}`, name: "Release" } });
    const run = await prisma.energyAnalysisRun.create({ data: { userId: user.id, energySiteId: site.id, status: "RUNNING", engineVersion: "TEST", methodologyVersion: "TEST", inputFingerprint: suffix, startedAt: now } });
    const job = await prisma.scheduledJob.create({ data: { type: ENERGY_ANALYSIS_JOB, idempotencyKey: `energy-analysis:${run.id}`, payload: { version: 2, analysisRunId: run.id }, status: "RUNNING", runAt: now, lockedAt: now, attempts: 1 } });
    try {
      await expect(recoverStaleAnalysisJobs(now, [job.id])).resolves.toEqual({ scanned: 0, recovered: 0, failed: 0 });
