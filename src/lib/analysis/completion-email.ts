...
function baseRows(input: CompletionEmailInput) {
  ...
  const bestSelfUse = cheapest(current.filter((item) => item.controlMode === "SELF_USE"));
  const bestSmart = cheapest(current.filter((item) => item.controlMode === "SMART"));
  ...
  const saving = (item: CompletionEmailScenario | null) =>
    item && today && today !== item ? `úspora ${czk.format(Math.round(today.annualCostCzk - item.annualCostCzk))} Kč/rok` : null;
  const rows: Row[] = [];
  ...
  return { rows, today, best: bestSmart ?? bestSelfUse };
}
...
export function buildAnalysisCompletionEmail(input: CompletionEmailInput) {
  ...
  const subject =
    input.kind === "PRO"
      ...
      : base.today && base.best && base.best !== base.today
