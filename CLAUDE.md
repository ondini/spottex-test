...
The SolaX key agent (`scripts/solax-key-agent/`, compose profile `solax-key-agent`, port `3011`) lives under the same hosting and token rules. It is a self-healing extractor of the per-account SolaX Cloud `tokenID`: `POST /extract` runs the current Playwright script, and when the portal UI changes and the run fails, the agent hands Codex the failure screenshot, DOM and console, validates the repaired script with a live run, and only then promotes it (previous versions are archived in the state volume). Repaired scripts execute with portal credentials inside this container, which is exactly why it carries no secret other than its own Codex session — never add platform database or encryption env vars to it. Engine logic is unit-tested in Vitest; the full loop is covered by the opt-in `node scripts/solax-key-agent/selftest.mjs` (fixture portal + codex stub), which is not part of CI because it needs Playwright's chromium. The backend's `/sync_inverter` Selenium scraper is the intended future caller.
