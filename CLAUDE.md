...
The SolaX key agent (`scripts/solax-key-agent/`, compose profile `solax-key-agent`, port `3011`) lives under the same hosting and token rules. It is a self-healing extractor of the per-account SolaX Cloud `tokenID`; `POST /extract` returns `{tokenId, scriptVersion, discovered}`. Login is a stable primitive (`login.mjs`) run once per session, because SolaX locks accounts that log in too often — the discovery loop reuses that one authenticated session rather than logging in again. The volatile "where is the token" logic is an isolated post-login locator (`locator.default.mjs`, then `current.mjs` in the state volume). When the locator fails, the agent explores the authenticated portal (`explorer.mjs`: clicks nav including new tabs, collects rendered token-shaped strings with their labels, page text and screenshots), hands that evidence to Codex, gets a new locator, validates it in the same session, and only then promotes it (prior versions archived). The app renders the tokenID into the DOM after decrypting it, so exploration that reaches the right page sees it — the old script only failed because the token moved pages. Locators run with portal credentials inside this container, which is why it carries no secret other than its own Codex session — never add platform database or encryption env vars to it. Engine logic (promote/archive, discovery-on-failure) is unit-tested in Vitest; the full exploration loop is covered by the opt-in `node scripts/solax-key-agent/selftest.mjs` against two fixture portals (old = locator works directly; new = token relocated to another tab, so the seed fails and discovery must find it) with a codex stub, not in CI because it needs Playwright's chromium. The backend's `/sync_inverter` Selenium scraper is the intended caller; it can be driven in batch for new plants that still lack a key.
